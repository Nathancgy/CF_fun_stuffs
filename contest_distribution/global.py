...
months = generate_month_list(2019, 1, 2024, 10)
...
with open('output.csv', 'r', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        ...
        # Check if the contest is rated and determine divisions
        if 'global' in contest_name_lower:
...
for year in sorted(yearly_data.keys()):
    ...
    ax.text(year_mid, ax.get_ylim()[1] + 1, str(year), ha='center', va='bottom', fontsize=10)
    # Add a vertical line to separate years
    ax.axvline(x=year_end + 1, color='gray', linestyle='--', linewidth=0.5)

# Adjust the plot limits
ax.set_ylim(0, max(div_bottoms) + 5)

# Add labels and title
ax.set_xlabel('Month')
ax.set_ylabel('Number of Rated Contests')
ax.set_title('Monthly Distribution of Codeforces Rated Contests by Division (2012-2024)')
